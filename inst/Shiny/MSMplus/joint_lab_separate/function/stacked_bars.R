...
stacked_function_bars<-function(json, data, labels_cov, labels_state ) {
  ...
  for (k in 1:length(json$atlist)) {
    ...
    pro_stack2$cov=as.character(rep(json$atlist[k],nrow(pro_stack2)))
    
    pro_stack3=matrix(nrow=nrow(pro_stack2),ncol=ncol(pro_stack2)-2,NA)
    
    pro_stack3[,1]=pro_stack2[,1]
    
    for (i in 1:(length(json$P))) {
      pro_stack3[,i]=pro_stack2[,i]
    }
    ...
    for (o in 1:(length(json$P))) {
      for (g in 1:nrow(stack_long))  {
        if  (stack_long$state[g]==o) {stack_long$state_factor[g]=labels_state[o]  }  
      }
    }

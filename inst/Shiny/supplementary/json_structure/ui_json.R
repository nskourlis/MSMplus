          ...
          fluidRow(                  
            tags$head(
              ...
              tags$style(type = "text/css", ".form-control {display: block; width: 100%;height: calc(1.5em + 0.75rem + 2px);padding: 0.375rem 0.75rem;
                    font-size: 2rem !important;font-weight: 400;line-height: 1.5;color: #495057;background-color: #fff;background-clip: padding-box;
                    border: 1px solid #ced4da; border-radius: 0.25rem;-webkit-transition: border-color 0.15s ease-in-out, -webkit-box-shadow 0.15s ease-in-out;
              ...
              tags$style(type = "text/css", ".btn {display: inline-block;font-weight: 400;color: #495057;text-align: center;vertical-align: middle;
                    ...
                    border: 1px solid transparent;padding: 0.375rem 0.75rem;font-size: 2rem !important;line-height: 1.5;border-radius: 0.25rem;
              ...
              tags$style(type = "text/css", ".custom-file-input::before {content: 'Select file';display: inline-block;background: linear-gradient(top, #f9f9f9, #e3e3e3);
                                border: 1px solid #999;border-radius: 3px;padding: 5px 8px;outline: none;white-space: nowrap;-webkit-user-select: none;
                                cursor: pointer;text-shadow: 1px 1px #fff;font-weight: 700; font-size: 15pt !important;}"),
              
            ),  
            
            
            column(12,uiOutput("to_url_backjson"),
                   tabsetPanel(
                     tabPanel(h2("Json file"),
                              uiOutput("message_json")
                              ),
                     tabPanel(h2("Descriptive json file"),
                              uiOutput("message_json1"),
                              verbatimTextOutput("json_file1")
                              ),
                     tabPanel(h2("Results json file"),    
                              uiOutput("message_json2"),
                              verbatimTextOutput("json_file2")
                              )
                    )
           )

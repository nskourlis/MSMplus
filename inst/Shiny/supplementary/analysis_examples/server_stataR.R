...
output$message_analysis_R<-renderUI({ 
  ...
  intro =p(
    withMathJax(
      ...
      helpText(''),
      helpText(HTML('&emsp;') ,'msboxes_R(data=msebmt,id= msebmt$id, yb=c(0.3,0.5,0.75),xb=c(0.5,0.2,0.7),boxwidth=0.1,boxheight=0.1,'),
      ...
      helpText('#In our example we are interested in making predictions for 3 covariate patterns- 3 age groups'),
      helpText(HTML('&emsp;') ,'flexsurvjson(model=cffpm.list, vartime= tgrid, qmat=tmat,process="Markov",'),
